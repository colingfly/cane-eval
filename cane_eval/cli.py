...
def _trait_bar_cli(trait, score, width=20):
    """Render a trait bar for CLI output."""
    filled = int(score / 100 * width)
    empty = width - filled
    if trait in ("overconfidence", "hedging") and score > 60:
        color = "red"
    elif score >= 70:
        color = "green"
    elif score >= 40:
...
def print_profile(profile):
    """Print personality profile to terminal."""
    print()
    print(c("  =" * 40, "dim"))
    print()
    print(f"  {c('Agent Personality Profile', 'bold')}  {c(profile.suite_name, 'cyan')}")
    print(f"  {profile.total_results} responses | {profile.embedding_model} | {profile.projection_method}")
    print()

    # Personality traits
    if profile.personality and profile.personality.trait_scores:
        print(f"  {c('Traits:', 'bold')}")
        for trait, score in profile.personality.trait_scores.items():
            bar = _trait_bar_cli(trait, score)
            badge = ""
            if trait in profile.personality.risk_traits:
                badge = f" {c('RISK', 'red')}"
            elif trait in profile.personality.dominant_traits[:2]:
                badge = f" {c('DOMINANT', 'cyan')}"
            print(f"    {trait:16s} {bar} {_score_color(score)}{badge}")
        print()

    # Clusters
    if profile.clusters:
        print(f"  {c('Clusters:', 'bold')}")
        for cid, label in profile.clusters.items():
            print(f"    {c(f'[{cid}]', 'cyan')} {label}")
        print()
    ...
    if profile.steering_vectors:
        ...
        for sv in profile.steering_vectors:
            mag_color = "green" if sv.magnitude > 0.5 else "yellow" if sv.magnitude > 0.2 else "dim"
            print(f"    {c(sv.name, 'cyan')}  magnitude: {c(f'{sv.magnitude:.3f}', mag_color)}")
            print(f"      {sv.negative_label} ({sv.n_negative}) {c('←→', 'dim')} {sv.positive_label} ({sv.n_positive})")
        print()

    # Contrastive pairs
    if profile.contrastive_pairs:
        print(f"  {c(f'{len(profile.contrastive_pairs)} contrastive pairs', 'bold')} extracted")
        print()
...
def cmd_profile(args):
    """Run personality profiling on eval results or a suite."""
    from cane_eval.profiler import Profiler
    ...
    if args.results:
        ...
        if not args.quiet:
            print()
            print(f"  {c('cane-eval profile', 'cyan')} {c(args.results, 'bold')}")
            print()

        profile = profiler.profile_from_json(args.results)
    else:
        # Run eval first, then profile
        from cane_eval.suite import ReliabilitySuite
        from cane_eval.engine import ReliabilityRunner
        ...
        if not args.quiet:
            print()
            print(f"  {c('cane-eval profile', 'cyan')} {c(suite.name, 'bold')}")
            print(f"  {len(suite.tests)} test cases | model: {suite.model}")
            print()
            print(f"  Running eval first...")
            print()
    ...
    if args.output_json:
        ...
        print(f"  Profile saved to {args.output_json}")
    ...
    if args.export_vectors:
        ...
        vectors_data = [sv.to_dict() for sv in profile.steering_vectors]
        with open(args.export_vectors, "w") as f:
            _json.dump(vectors_data, f, indent=2)
        print(f"  Steering vectors saved to {args.export_vectors}")
...
def main():
    ...
    profile_parser.add_argument("--tags", help="Comma-separated tag filter")
    profile_parser.add_argument("--provider", default="anthropic", help="Judge provider")
    profile_parser.add_argument("--base-url", help="Base URL for OpenAI-compatible endpoints")
    profile_parser.add_argument("--embedding-model", default="all-MiniLM-L6-v2", help="Sentence-transformers model for embedding (default: all-MiniLM-L6-v2)")
    profile_parser.add_argument("--clusters", type=int, default=4, help="Number of behavioral clusters (default: 4)")
    profile_parser.add_argument("--projection", choices=["auto", "umap", "pca"], default="auto", help="Dimensionality reduction method (default: auto)")
    profile_parser.add_argument("--output-json", help="Save profile as JSON")
    profile_parser.add_argument("--html", help="Generate interactive HTML report")
    profile_parser.add_argument("--export-vectors", help="Export steering vectors as JSON")
    profile_parser.add_argument("--quiet", "-q", action="store_true", help="Minimal output")
    ...
    if args.command == "run":
        ...
        if not args.suite and not args.results:
            print(c("  Error: provide a suite YAML or --results JSON", "red"))
            sys.exit(1)
        cmd_profile(args)
